...
class TModule(nn.Module):
    def __init__(self, d_model, head, dim_feedforward=1024, dropout=0.1):
        ...
        self.self_attn = MultiHeadAttention2(d_model, head, dropout=0)  # 90 30 200 token
    ...
    for epoch in range(max_epoch):
        ...
        for b in range(batch_epoch):
            ...
            if b == batch_epoch-1:
                ...
                # loss = F.cross_entropy(output, train_label[b * batch:], weight=torch.tensor([0.7, 1]).to(device))
                loss = F.cross_entropy(output, train_label[b * batch:]) + 0.00001*at

            else:
                output, at = model(train_data[b * batch:(b + 1) * batch],
                                   )
                # loss = F.cross_entropy(output, train_label[b * batch:(b + 1) * batch], weight=torch.tensor([0.7, 1]).to(device))
        ...
        #     print('train:', train_result)
            ...
            # if epoch > 65:
            #     torch.save(model.state_dict(), 'draw/interpretation/mdd_stage/dropout0/epoch' + str(epoch) + '_cv' + str(i) + '_checkpoint.pt')
    return rr, model
...
if __name__ == '__main__':
    ...
    data = np.load('data/MDD/MDD_HC_sig.npy')  # 427 950 116
    label = np.load('data/MDD/MDD_HC_label.npy')  # 427
